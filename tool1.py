...
st.set_page_config(page_title="🔍 Complete SEO Insights Extractor (Ahrefs)", layout="wide")

# --- Ahrefs Credentials & Config (for Tab 1) ---
# IMPORTANT: Replace 'YOUR_AHREFS_API_TOKEN_HERE' with your actual Ahrefs API token.
# You can find your token in your Ahrefs account settings.
AHREFS_API_TOKEN = "O7FaWJ_lWMjqVVxZ50gDCgELSmr1vQQpCW1Ux3zt" 
AHREFS_BASE_URL = "https://api.ahrefs.com/v3"

# --- Tab Setup ---
tab1, tab2, tab3 = st.tabs(["🔍 SEO Analysis (Ahrefs)", "🎯 Smart Anchor & URL Matcher", "📖 About / How To Use"])

# ================================
# TAB 1: SEO ANALYSIS (Ahrefs Integration)
# ================================
with tab1:
    st.title("🔍 Complete SEO Insights Extractor (Ahrefs)")
    st.markdown("Extract **Keywords**, **Backlinks**, and **Domain Rankings** for your URLs using Ahrefs API.")
    ...
    def ahrefs_request(endpoint, params, max_retries=3):
        """Generic Ahrefs API GET request function with token authentication"""
        if AHREFS_API_TOKEN == "YOUR_AHREFS_API_TOKEN_HERE":
            st.error("Please provide your Ahrefs API Token in the script's `AHREFS_API_TOKEN` variable.")
            return None

        url = AHREFS_BASE_URL + endpoint
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    if data.get("error"):
                        # Ahrefs API might return a 200 OK with an error object if token is invalid or query is bad
                        st.warning(f"Ahrefs API Error: {data['error'].get('message', 'Unknown error')}")
                        return None
                    return data
                else:
                    st.warning(f"Ahrefs API HTTP Error {response.status_code}: {response.text}")
                time.sleep(1 + attempt) # Exponential backoff
            except requests.exceptions.RequestException as e:
                st.error(f"Request failed (attempt {attempt + 1}/{max_retries}): {str(e)}")
                time.sleep(2 + attempt) # Longer backoff for connection errors
        return None

    def fetch_keywords(domain):
        """Fetch top organic keywords for a domain using Ahrefs."""
        try:
            # Ahrefs Site Explorer's "organic-keywords" endpoint
            params = {
                "target": domain,
                "output": "json",
                "limit": 5, # Fetch top 5 keywords
                "metrics_type": "sum" # Use sum for metrics across all versions (http/s, www/non-www)
            }
            data = ahrefs_request("/site-explorer/organic-keywords", params)
            ...
            st.error(f"Error fetching keywords for {domain}: {e}")
            return []

    def fetch_backlinks(domain):
        """Fetch backlinks for a domain using Ahrefs."""
        try:
            # Ahrefs Site Explorer's "all-backlinks" endpoint (corrected from previous "/backlinks")
            params = {
                "target": domain,
                "output": "json",
                "limit": 3, # Fetch top 3 backlinks
                "order_by": "link_traffic:desc", # Order by estimated traffic from link
                "metrics_type": "sum"
            }
            data = ahrefs_request("/site-explorer/all-backlinks", params) # Corrected endpoint
            if not data or not data.get("results"): return {"total_count": 0, "items": []}

            backlinks = []
            for item in data["results"]:
                backlinks.append({
                    "domain_from": urlparse(item.get("source_page", "")).netloc,
                    "anchor": item.get("anchor_text", ""),
                    "url_from": item.get("source_page", ""),
                    "url_to": item.get("target_page", "")
                })
            # Ahrefs API 'backlinks' endpoint doesn't directly give total_count in the same response
            # We'd typically get total backlinks from the metrics endpoint.
            # For simplicity, we'll return a placeholder here or rely on fetch_domain_metrics for total count.
            return {"total_count": 0, "items": backlinks} # total_count will be updated by domain_metrics

        except Exception as e:
            st.error(f"Error fetching backlinks for {domain}: {e}")
            return {"total_count": 0, "items": []}

    def fetch_domain_metrics(domain):
        """Fetch domain ranking and overall metrics using Ahrefs."""
        try:
            # Ahrefs Site Explorer's "metrics" endpoint
            params = {
                "target": domain,
                "output": "json",
                "metrics": "domain_rating,ahrefs_rank,backlinks_total,refdomains_total",
                "mode": "subdomains", # Include subdomains for overall domain metrics
                "history_mode": "last"
            }
            data = ahrefs_request("/site-explorer/metrics", params)
            if not data or not data.get("results"): return {}
            
            metrics = data["results"][0] if data["results"] else {}
            return {
                "domain_rating": metrics.get("domain_rating", 0),
                "ahrefs_rank": metrics.get("ahrefs_rank", 0),
                "total_backlinks": metrics.get("backlinks_total", 0),
                "total_referring_domains": metrics.get("refdomains_total", 0),
            }
        except Exception as e:
            st.error(f"Error fetching domain metrics for {domain}: {e}")
            return {}

    def process_url(url):
        """Process a single URL and extract all Ahrefs SEO insights"""
        domain = extract_domain(url)
        results = {"original_url": url, "domain": domain}
        with st.spinner(f"🔍 Analyzing {domain} with Ahrefs..."):
            ...
            # Update total backlinks from domain_metrics for consistency
            if results["domain_metrics"]:
                results["backlinks"]["total_count"] = results["domain_metrics"].get("total_backlinks", 0)
            time.sleep(0.5) # Be mindful of Ahrefs API rate limits
        return results
    ...
    def format_results_for_csv(all_results):
        ...
        for res in all_results:
            row = {
                ...
                "Domain": res["domain"],
                "Ahrefs Domain Rating": res["domain_metrics"].get("domain_rating", 0),
                "Ahrefs Rank": res["domain_metrics"].get("ahrefs_rank", 0),
                "Total Backlinks": res["backlinks"].get("total_count", 0),
                "Total Referring Domains": res["domain_metrics"].get("total_referring_domains", 0),
            }
            # Add top keywords
            for i in range(3):
                kw = res["keywords"][i] if i < len(res["keywords"]) else {}
                row[f"Top Keyword {i+1}"] = kw.get("keyword", "")
                row[f"Keyword {i+1} Volume"] = kw.get("search_volume", 0)
                row[f"Keyword {i+1} Position"] = kw.get("position", 0)
            # Add top backlinks
            for i in range(2):
                bl = res["backlinks"].get("items", [])
                link = bl[i] if i < len(bl) else {}
                row[f"Top Backlink {i+1} From Domain"] = link.get("domain_from", "")
                row[f"Top Backlink {i+1} Anchor"] = link.get("anchor", "")
                row[f"Top Backlink {i+1} Source URL"] = link.get("url_from", "")
    ...
    if uploaded_file:
        ...
        if st.button("🚀 Start SEO Analysis (Ahrefs)", type="primary", key="start_seo_ahrefs"):
            urls = df[url_column].dropna().unique().tolist()
            if len(urls) > 10:
                st.warning(f"⚠️ Found {len(urls)} URLs. Processing the first 10 for this demo to save API credits and time.")
                urls = urls[:10]
            ...
            status_text.text("✅ Ahrefs analysis complete!")
            if all_results:
                results_df = pd.DataFrame(format_results_for_csv(all_results))
                st.markdown("### 📋 Final Results Summary")
                st.dataframe(results_df)
                csv_output = results_df.to_csv(index=False).encode('utf-8')
                st.download_button("📥 Download Results as CSV", csv_output, "ahrefs_seo_analysis.csv", "text/csv", type="primary")

# ====================================================
# TAB 2: SMART ANCHOR & URL MATCHER (Unchanged Logic)
# ====================================================
with tab2:
    ...
    **Find the most relevant client page for any linking opportunity.** This tool analyzes the content of a source page and compares it against a list of your client's pages to recommend the best match and suggest ideal anchor texts.
    ...
    def extract_content_from_url(url, timeout=10):
        ...
        try:
            ...
            st.warning(f"Failed to extract content from {url}: {e}")
            return None
    ...
    def find_best_client_page_matches(source_df, source_url_col, client_urls, max_rows=None):
        ...
        if not client_pages_content:
            st.error("Could not extract content from any client URLs. Please check the URLs and ensure they are publicly accessible.")
            return []
...
# ================================
# TAB 3: ABOUT / HOW TO USE (Updated)
# ================================
with tab3:
    ...
    st.header("🔍 SEO Analysis (Ahrefs)")
    st.markdown("""
    This tool provides a high-level overview of the SEO profile for a list of URLs using the **Ahrefs API**.
    
    **How to use:**
    1.  **Important:** Before running, you need to set your Ahrefs API Token in the `AHREFS_API_TOKEN` variable within the script.
    2.  Prepare a CSV file with a list of URLs you want to analyze. Make sure the file has a header row.
    3.  Upload the CSV file using the uploader.
    4.  Select the column from your file that contains the URLs.
    5.  Click **"Start SEO Analysis (Ahrefs)"**.
    
    **What you get:**
    - **Ahrefs Domain Rating**: Ahrefs' proprietary metric indicating the strength of a domain's backlink profile.
    - **Ahrefs Rank**: The global rank of the website based on its backlink profile size and quality.
    - **Total Backlinks**: The total number of backlinks pointing to the domain.
    - **Total Referring Domains**: The total number of unique domains linking to the target.
    - **Top Keywords**: The top organic keywords the domain ranks for, along with their search volume and position.
    - **Top Backlinks**: A sample of recent or strongest backlinks, showing the linking domain and anchor text.
...
# --- Footer ---
st.markdown("---")
st.markdown("<div style='text-align: center; color: grey;'>Built with Streamlit, Pandas, and scikit-learn</div>", unsafe_allow_html=True)
